...
export const buildSearchQueryFilter = (params: SearchParams): string => {
  ...
  if (name && name.trim()) {
    ...
    if (isLikelyCardNumber(name)) {
      ...
      if (cardNumberFilter) {
        conditions.push(`(name ILIKE '%${name.replace(/'/g, "''")}%' OR ${cardNumberFilter} OR card_number ILIKE '%${name.replace(/'/g, "''")}%')`);
      } else {
  ...
  // Handle specific card number search - now we can use both the JSON attributes and the dedicated column
  if (cardNumber && cardNumber.toString().trim()) {
    const cardNumberFilter = generateCardNumberSearchFilter(cardNumber);
    if (cardNumberFilter) {
      conditions.push(`(${cardNumberFilter} OR card_number ILIKE '%${cardNumber.toString().replace(/'/g, "''")}%')`);
    }
  }
 ...
 */
export const buildSearchSortOptions = (params: SearchParams) => {
