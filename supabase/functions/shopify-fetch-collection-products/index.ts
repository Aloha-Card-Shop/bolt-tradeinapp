...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Starting background sync for collection: ${collection.shopify_collection_id}`);
    ...
    const backgroundTask = async () => {
      try {
            ...
            for (const variant of variants) {
              ...
              if (existingInventory && existingInventory.length > 0) {
                ...
                if (inventoryError) {
                  ...
                  createdCount++;
                  console.log(`Created inventory record for: ${cardName}`);
                }
              }
            }
          }
        ...
        console.log(`Finished processing ${allProductIds.length} products with ${totalVariants} total variants`);
        console.log(`Summary: ${existingCount} existing, ${createdCount} created, ${failedProducts.length} failed`);
      } catch (error) {
    ...
    return new Response(
      JSON.stringify({
        message: `Background sync started for collection "${collection.title}"`,
        collection: {
          id: collectionId,
